...
tts = sapply(unique(tlx$tlxVariable), function(var) {
  ...
  tlx_ss_u = tlx_ss %>% filter(mode == 'Unrestricted') %>% .$tlxValu
  diff = mean(tlx_ss_r) - mean(tlx_ss_u)
  tt = t.test(tlx_ss_r, tlx_ss_u)
  c(as.character(var), mean(tlx_ss_r), mean(tlx_ss_u), diff, tt$p.value, as.numeric(tt$parameter), tt$statistic)
}) %>%
  t %>%
  as.data.frame %>%
  mutate(
...
colnames(tts) = c('tlxVariable', 'r', 'u', 'diff', 'p', 'df', 't')
tts = tts %>%
...
save_plot("~/Dropbox/Cambridge/R230/Report/tlx_results.pdf", p, base_height = 4.5)
p
```
...
```
```{r}
nutt = utts %>% group_by(id, mode) %>% summarise(n = n())
r_nutt = nutt %>% filter(mode == 'Restricted') %>% .$cTokens
u_nutt = nutt %>% filter(mode == 'Unrestricted') %>% .$cTokens
